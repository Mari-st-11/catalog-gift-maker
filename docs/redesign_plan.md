...
- [ ] デザインテーマ（mode/botanical/nuance）に応じた画面デザインの作り直し
      ...
      **贈り主側（ログイン必須）**
      ...
      8. [済(枠のみ)] ギフトアイテム新規登録 (`/gift_lists/:uuid/gift_items/new`) — URL入力 or 手動入力(ウィザードのステップ2)。
         ヘッダー・ステップインジケーターのみ反映済み、中身(検索UI等)は未着手
      ...
      **その他**
      ...
      2026-08-23時点で、上記のうち「8のギフトアイテム新規登録画面の中身(検索UI・URL入力UIそのもの)」
      以外は全画面のデザイン反映が完了。トップページの横スクロールカルーセルはPCだとスワイプしづらい
      という指摘を受け、`md:`以上では横スクロールをやめて折り返しグリッド表示に変更した。
