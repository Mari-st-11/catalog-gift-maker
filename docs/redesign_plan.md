...
- [ ] デザインテーマ（mode/botanical/nuance）に応じた画面デザインの作り直し
      ...
      並行して作成中。
      対象画面は以下の14画面（2026-08-22時点で洗い出し済み、[済]は実装済み）:
      **認証まわり**
      1. [済] ログイン画面 (`/users/sign_in`) — メール/パスワード + Google/LINEログインボタン
      2. [済] アカウント編集画面 (`/users/edit`) — 表示名・メールアドレスの変更、アカウント削除
      3. パスワード再設定画面 (`/users/password/new`, `/edit`) — リンクは非表示だがルートは生きている（優先度低）
      　※新規登録画面(`/users/sign_up`)はGoogle/LINE誘導のみのブロック画面のため対象外
      **贈り主側（ログイン必須）**
      4. [済] ギフトリスト一覧 (`/gift_lists`) — 未ログイン時は誘導表示、ステータス別バッジ表示
      　　15. （追加予定）履歴一覧 — `completed`のギフトリストを絞り込み表示、いつ・誰に・何を贈ったか
      5. [済] ギフトリスト新規作成 (`/gift_lists/new`) — ウィザードのステップ1(相手を決める)。
         作成後はステップ2(ギフト追加)へ遷移するようリダイレクト先を変更
      6. [済] ギフトリスト詳細/管理画面 (`/gift_lists/:uuid`) — 商品一覧、共有URLコピー、リンク再発行、選ばれた商品の確認、「選び直してもらう」ボタン
      7. [済] ギフトリスト編集 (`/gift_lists/:uuid/edit`) — リスト名・メッセージ編集
      8. [済] ギフトアイテム新規登録 (`/gift_lists/:uuid/gift_items/new`) — 検索/URL入力/手動入力の3タブUI(ウィザードのステップ2)。
         追加後は同画面に留まり「選択済みの商品」一覧に積み上がる形に変更
      9. [済] ギフトアイテム編集 (`/gift_items/:id/edit`)
      10. [済] ギフトアイテム詳細/管理画面側 (`/gift_items/:id`) — あまり使われていないが一覧の遷移先として存在
          - [ ] （UI改善予定）別の編集画面(`/gift_items/:id/edit`)へ遷移させず、
            詳細画面(この画面)の中で直接編集できるようにする
      **受け取り主側（ログイン不要・公開ページ）**
      11. [済] 公開カタログ画面 (`/shared_gift_lists/:token`) — テーマに応じた見た目、商品一覧、選択ボタン、メッセージ表示、確定後の案内
      12. [済] 公開アイテム詳細 (`/shared_gift_lists/:token/shared_gift_items/:id`) — 商品名・画像・説明の詳細ページ
      **その他**
      13. [済] トップページ (`/`) — サービス紹介、「はじめる」CTA、使い方カルーセル
      14. [済] 共通ヘッダー/フッター（ログイン状態に応じた表示切り替え）
      15. [済] 通知一覧画面 (`/notifications`)

      2026-08-23時点で、8を含む全14画面のデザイン反映が完了。トップページの横スクロールカルーセルは
      PCだとスワイプしづらいという指摘を受け、`md:`以上では横スクロールをやめて折り返しグリッド表示に変更した。
      画面8は検索・URL入力・手動入力をタブで切り替えるUIに変更し、追加後はステップ2画面に留まって
      「選択済みの商品」一覧に積み上がる形にした（手動入力時に入力済みの商品名・説明がプレースホルダーで
      上書きされてしまうバグも合わせて修正）。
